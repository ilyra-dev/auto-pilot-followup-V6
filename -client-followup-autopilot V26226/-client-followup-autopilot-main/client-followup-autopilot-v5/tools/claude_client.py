...
def _build_system_prompt(company_name, language, stage, style_examples=None):
...
Keep the email under 150 words. Be helpful and professional.

IMPORTANT: Return ONLY valid JSON with keys "subject" and "body_html".
The body_html should be clean HTML suitable for email (use <p>, <br>, <strong> tags).
...
def generate_followup_email(context, language, stage, company_name="", style_examples=None):
    """
    ...
    system_prompt = _build_system_prompt(company_name, language, stage, style_examples)
...
The email should ask the client to provide the specific information/documents listed in "Information Needed".
Do NOT reference the internal deliverable name.
